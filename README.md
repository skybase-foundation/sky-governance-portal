...
- Set `USE_CACHE` to true if you want to use cache. With `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` set (or an Upstash `REDIS_URL`, from which they are derived) it uses Upstash over HTTP, otherwise the filesystem cache. The cache is always skipped during `next build`.
