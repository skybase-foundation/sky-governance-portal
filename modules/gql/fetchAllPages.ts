...
// and reports nothing when it does. Pages must not exceed the cap, or a full page would look short.
export const INDEXER_PAGE_SIZE = 1000;
export const MAX_INDEXER_PAGES = 100;

// Walks a keyset-paginated query (order_by id asc, id _gt cursor) until a page comes back short.
...
export async function fetchAllPages<T extends { id: string }>(
  ...
  for (let page = 0; page < MAX_INDEXER_PAGES; page++) {
    ...
    rows.push(...pageRows);
    if (pageRows.length < INDEXER_PAGE_SIZE) return rows;
